...
@section('content')
    <div class="text-center mb-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Buat Password Baru</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Silakan masukkan password barumu di bawah ini.</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.update') }}" class="space-y-5">
        ...
        <div>
            ...
            <input type="email" name="email" value="{{ $email ?? old('email') }}" readonly class="w-full px-4 py-2 bg-slate-100 dark:bg-darkBg/30 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-500 dark:text-slate-400 outline-none cursor-not-allowed">
